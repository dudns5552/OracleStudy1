...
trim() : 공백을 제거할 때 사용
    형식]
        trim([leading | trailing | both] 제거할문자 from 컬럼)
        - leading  : 좌측의 공백 제거
        - trailing : 우측
...
/*
replace()
    문자열을 다른 문자열로 대체할 때 사용한다. 만약 공백으로
    문자열을 대체한다면 해당 문자열을 삭제하는 결과가 된다.
    형식]
        replace(컬럼명 or 문자열, '변경 대상의 문자', '변경할 문자')

※trim(), ltrim(), rtrim()의 기능을 replace() 하나로 대체할 수
