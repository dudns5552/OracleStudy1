...
/*
sysdate
    현재 날짜와 시간을 초 단위로 반환해준다. 주로 게시판이나 회원가입시
     날짜를 입력하기 위해 사용된다. 오라클의 기본 서식이 xx/xx/xx이므로 
     출력시 시간은 표시되지 않지만, 서식문자를 통해 초단위까지 표현할 수 있다.*/
select sysdate from dual;


/*
날짜포맷 
    오라클은 대소문자를 구분하지 않으므로, 서식문자 역시 구분없이 사용할 수 있다.
...
이를 제외한 나머지 문자열은 "으로 묵어줘야한다. 서식문자를 감싸는게 아니므로
작성에 주의해야한다. */
...
select to_char('2021"년"01"월"01"일"', 'yyyy-mm-dd')
from dual; --오답

select to_char('2021"년"01"월"01"일"', 'yyyy"년"mm"월"dd"일"')
from dual; --오답

select to_char('2021년01월01일', 'yyyy"년"mm"월"-dd"일"')
from dual; --오답

select to_date('2021"년"01"월"01"일"', 'yyyy-mm-dd')
from dual; --오답

select to_date('2021"년"01"월"01"일"', 'yyyy"년"mm"월"dd"일"')
from dual; --오답

select to_date('2021년01월01일', 'yyyy"년"mm"월"dd"일"')
from dual; --정답


select to_char(to_date('2021년01월01일', 'yyyy"년"mm"월"dd"일"'),
'yyyy"년 "mm"월 "dd"일은 "dy"요일"') from dual; --정답!
...
select
    to_date('2021년01월01일', 'yyyy"년"mm"월"dd"일"') "1. 날짜인식",
    to_char(to_date('2021년01월01일', 'yyyy"년"mm"월"dd"일"'), 'day') "2. 요일인식"
from dual;
...
*/
select ename, substr(hiredate, 1, 2) 년도, 
replace(substr(hiredate, 3, 3), '/', '') 입사월,
...
단, 날짜의 형태는 ‘01-01-2020’ 포맷으로 사용한다. 즉 sysdate - ‘01-01-2020’ 이와같은 
연산이 가능해야한다. 
*/
select sysdate, 
