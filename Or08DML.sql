...
레코드 입력하기 : insert
    ...
    숫자형은 '없이 기술한다. 만약 숫자형을 '로 감싸게 되면 실행시
    자동으로 변환되어 입력된다.
*/
...
여기서 말하는 외부란 Java/JSP와 같은 Oracle 이외의 프로그램을 만한다.*/
commit;

--커밋 이후 새로운 레코드를 삽입하면 임시테이블에 저장된다.
insert into tb_sample (no, loc, manager) values (60, '태국', '손오공');
--select 명령으로 확인할 수는 있지만 실제 테이블에는 반영이 되지않은 상태이다.
select * from tb_sample;
--롤백 명령으로 마지막 커밋 상태로 되돌릴 수 있다.
rollback;
--마지막 커밋 이후 삽입된 '손오공'은 제거된다.
select * from tb_sample;
