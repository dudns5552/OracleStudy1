...
begin
    ...
    /*
    여러 조건을 사용할 때 Java와는 달리 else if가 아니라 elsif로
    기술해야한다. 또한 중괄호 대신 then과 end if를 사용한다. */
    if emp_dept = 50 then
        dept_name := 'Shipping';
...
반복문2 : while문
    ...
    즉, 조건에 맞지 않았다면 한번도 실행되지 않을 수 있다.
...
/*
예제14] 제어문(반복문 : while)
시나리오] while loop문으로 1부터 10까지의 정수의 
합을 구하는 프로그램을 작성하시오.
*/
declare
    i number := 1;
...
for문에 reverse를 추가하면 거꾸로 반복 할 수 있다.*/
begin
...
declare
    --문자형 변수 생성
    total1 varchar2(1000);
begin
     for dan in 2 .. 9 loop   
        for gob in 1 .. 9 loop
            --하나의 단씩 문자 변수에 누적해서 연결해준다.
            total1 := total1 ||dan ||' * ' || gob ||' = '|| (dan * gob) ||' ';
        end loop;
    --하나의 단이 완성된 문자열을 출력
...
/*
커서(Cursor)
    select 쿼리에 의해 여러행이 반환되는 경우 각 행에 접근하기 위한
    객체
    ...
    선언방법]
        ...
        Open 커서명
            쿼리를 실행하라는 의미. 즉, Open할때 Cursor선언시
            select문이 실행되어 결과셋을 얻게된다. Cursor는
            그 결과셋의 첫번째행에 위치하게 된다.
...
생성하시오. key는 숫자, value는 full_name 으로 지정하시오.
*/
...
시나리오] 100번 부서에 근무하는 사원의 사원번호를 인출하여 
VArray에 저장한 후 출력하는 PL/SQL을 작성하시오.
*/
