...
desc tb_primary3;
/* 이미 생성된 테이블에 제약조건을 부여할때는 alter를 사용하면된다.
...
/* 오류발생이 부모테이블에는 gildong 이라는 아이디가 없는 상태임.
...
create table tb_foreign2 (
    ...
    f_name varchar2(30),
    f_date date,
    /* tb_foreign2 테이블의 f_id 컬럼의 부모테이블이 tb_primary1의
    idx컬럼을 참조하는 외래키 생성 */
...
외래키 삭제시 옵션
[on delete cascade]
    부모 레코드 삭제시 자식레코드도 같이 삭제된다.
[on delete set null]
    부모 레코드 삭제시 자식레코드의 값이 null로 변경된다.
...
create table tb_not_null(
    ...
    pw varchar2(30) null, /* null 허용. 일반적으로 이렇게 쓰진 않는다.*/
    name varchar2(40) /* null허용. 일반적으로 이와같이 선언한다.*/
);
