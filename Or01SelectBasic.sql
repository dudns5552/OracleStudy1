...
select employee_id, first_name, last_name, email, department_id
    from employees; --하나의 쿼리문이 끝날때 반드시 ;을 명시해야한다.
...
select * from employees where last_name = 'Smith' and 
    salary = 8000;
    ...
    형식] 컬럼명 liek '%검색어%'
        ...
        % : 모든 문자 혹은 문자열을 대체
            Ex) D로 시작하는 단어 : D% -> Da, Dae, Daewoo
                Z로 끝나는 단어 : %Z -> aZ, abxZ, DragonballZ
                C가 포함되는 단어 : %C% -> aCb, abCde 광범위하게 쓰임
        _ : 언더바는 문자 하나를 대체
...
select * from employees where salary >= 8000 and
    department_id = 80;
select * from employees where salary >= 8000 and
...
*/
select ename, sal, deptno from emp where sal > 2000 and sal < 3000 and
    deptno = 20 or deptno = 30 order by ename;
/*
9. 1981년도에 입사한 사원의 이름과 입사일을 출력하시오. (like 연산자와 와일드카드 사용)
*/
select ename, hiredate from emp where hiredate like '81/_____';
/*
