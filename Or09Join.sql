...
파일명 : Or09Join.sql
테이블조인
설명 : 2개 이상의 테이블을 동싱 참조하여 데이터를 인출해야 할때
        사용하는 SQL문
***/
...
1] inner join (내부조인)
    ...
    - 일반적으로 기본키(Primary key)와 외래키(Foreign key)를 사용하여 
        join하는 경우가 대부분이다.
    - 2개의 테이블에 동일한 이름의 컬림이 존재해야하고, 이 경우 
        '테이블명.컬럼명' 과 같이 기술한다.
...
부서번호를 뜻하는 department_id가 양쪽 테이블 모두에 존재하므로 어떤 
테이블에서 가져와 인출해야할지 명시해야한다. */

--따라서 부서번호에 인출할 테이블명을 추가해준다.
select
    employee_id, first_name, last_name, email,
...
별칭을 명시해야한다. */
select 
    first_name 이름, last_name 성, email 메일주소,
    departments.department_id 부서코드, department_name 부서명,
    city 시, state_province 주,
    jobs.job_id 업무코드, job_title 업무명
from locations 
...
select 
    first_name 이름, last_name 성, email 메일주소,
    D.department_id 부서코드, department_name 부서명,
    city 시, state_province 주,
    J.job_id 업무코드, job_title 업무명
from locations L
...
from employees Em left outer join departments De
    on Em.department_id = De.department_id
where
    upper(hire_date) like '07/_____';

--선생님 답
--1. 저장된 레코드를 러프하게 확인한다.
...
select
    ...
    jobs.job_id, job_title, state_province
from locations 
...
/*
    ...
    즉, using은 테이블의 별칭 및 on절을 제거하여 보다 심플하게
...
select
    ...
    job_id, job_title, state_province
from locations 
    inner join departments using(location_id)
    inner join employees using(department_id)
    inner join jobs using(job_id)
...
select
    ...
    trim(to_char(salary, '$999,000')) 급여, department_name 부서명,
    country_id 국가코드, country_name 국가명
from employees
...
--문제에서 주어진 지역정보로 지역 일련변호를 확인(결과 : 1500)
select * from locations
where city = 'South San Francisco' and 
...
사원번호, 이름, 성, 급여, 부서명, 국가코드, 국가명(COUNTRY_NAME)
        급여는 세자리마다 컴마를 표시한다. */
select
    employee_id 사번, first_name 이름, last_name 성,
