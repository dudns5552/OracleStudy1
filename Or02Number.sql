...
설명 : 숫자데이터를 처리하기 위한 숫자관련 함수를 학습
        ...
        대상으로 한다.
*/
--tab : 현재 접속한 계정에 생성되어있는 테이블과 뷰의 목록을 보여준다.
select * from tab;
...
/*
trunc()
    소수점을 특정자리수에서 잘라낼때 사용
    형식]
        trunc(컬럼명 혹은 값, 소수점이하 자리수)
            두번째 인수가
                양수 : 주어진 숫자만큼 소수점을 표현
...
/*또는 영업사원은 커미션이 있으므로 commission_pct is not null;로도
인출할 수 있다.*/
...
select first_name, salary, trunc(commission_pct, 1), 
    salary +(salary*trunc(commission_pct, 1)) from employees 
    where job_id like 'SA_%';
    
--4. 계산식이 포함된 컬럼명에 별칭 부여
select first_name, salary, trunc(commission_pct, 1)as comm_pct, 
    salary +(salary*trunc(commission_pct, 1)) TotalSalary from employees 
    where job_id like 'SA_%';    
...
소수점 관련함수
    ...
    floor() : 무조건 버림처리
    round(값, 자리수) : 반올림 처리
        두번째 인수가 없는경우 : 소수점 첫째 자리가 5이상이면 올림,
                                미만이면 버림
                    있는경우 : 숫자만큼 소수점이 표현되므로 그 다음수가
                                5이상이면 올림, 미만이면 버림
*/
