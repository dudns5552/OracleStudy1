...
distinct : 동일한 값이 있는 경우 중복된 레코드를 제거한 후 하나의
            레코드만 가져와서 보여준다. 순수한 하나의 레코드이므로
            통계적인 값을 계산할 수 없다. */
select distinct job_id from employees;
/*
group by : 동일한 값이 있는 레코드를 하나의 그룹으로 묶어서 인출한다.
            보여지는 것은 하나의 레코드지만 다수의 레코드가 하나의
            그룹으로 묶여진 결과이므로 통계적인 값을 계산할 수 있다.
...
group by 절이 포함된 select 문의 형식
    ...
    from
        테이블
    where
        조건1 adn 조건2 or 조건n <- 물리적으로 존재하는 컬럼만 쓸수있다.
    group by
        레코드의 그룹화를 위한 컬럼(그룹화를 걸고싶은 컬럼)
    having
...
select
    trim(to_char(sum(salary), '$999,000')) as sumSalary
...
/*
    ...
    *를 사용하는 것을 권장한다. *는 컬럼의 특성 혹은 데이터에 따른
    방해를 받지 않으므로 실행속도가 빠르기 때문이다. */
...
select
    count(*) "전체사원수",
    sum(salary) "급여의 합계",
    round(sum(salary) / count(*)) "평균급여(직접계산)",
    round(avg(salary)) "avg함수사용",
...
/*
2. 컬럼 자체의 값을 대문자로 변한 후 쿼리의 조건으로 사용한다.
...
--따라서 2100불을 받는 직원을 인출하면 위 질물을 해결할 수 있다.
...
1단계 : 부서가 많은 경우 일일이 부서별로 확인할 수 없으므로 부서를 그룹화한다.
        중복이 제거도니 결과로 보이지만 동일한 레코드가 하나의 그룹으로 합쳐진
        결과가 인출된다. */
...
select
    department_id 부서번호,
    count(*) 사원수, --이렇게 써야 부서번호null인 사원도 카운트됨
...
group by 절에 2개의 컬럼을 명시해야한다. 즉 부서로 그룹화 한 뒤
다시 담당업무별로 그룹화한다. */
select
    department_id, job_id, count(*), 
    to_char(avg(salary), '$999,000')
from employees
...
추가하면 에러가 발생한다. 이 경우에는 having절에 조건을 추가해야한다.
    Ex) 급여가 3000인 사원 => 물리적으로 존재하므로 where절에 추가
        평균급여가 3000인 사원 => 상황에 맞게 논리적으로 만들어낸
                               결과이므로 having절에 추가해야한다.
*/
...
/* 관리자가 없는 사원은 물리적으로 존재하므로 where절에 기술
    ...
    select절에 사용한 가상의 컬럼(계산식 등)은 order by절에
    사용할 수 있음 */
...
*/
select department_id 부서번호, count(*), to_char(avg(salary), '$999,000.00') 평균급여
from employees
group by department_id
order by department_id;

/* 이와 같이 복잡한 계산식이 포함된 컬럼을 기준으로 정렬할때는 
별칭을 사용하는 것이 좋다.*/
