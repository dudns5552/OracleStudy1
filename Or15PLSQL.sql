...
declare
    ...
    => 변수명 자료형(크기);
    단, 기존에 생성된 컬럼의 타이과 크기를 참조하여 선언하는 것이 좋다.
...
/*
    ...
    호스트환경으로 빠져나오지 못한다. 즉 PL/SQL문장을 탈줄하기 위해
    필요하다.
...
/* 예제5] 복합변수
    ...
    만든 후 생성하는 변수를 말한다.
    형식]
        type 복합변수자료형명 is record (
            컬럼명1 자료형(크기),
            컬럼명2 테이블명.컬럼명%type
        );
    앞에서 선언한 자료형을 기본으로 변수 생성;
...
--PL/SQL 작성하기
declare 
    /* 인출시 데이터의 변화가 없는 3개의 컬럼은 기존 테이블을 
    참조해서 생성하고, 이름은 2개의 컬럼을 합쳐서 만드므로
    넉넉한 크기로 생성한다. */
    type empTypes is record(
        emp_id employees.employee_id%type,
        emp_name varchar2(50),
...
/*
치환연산자
    PL/SQL에서 사용자로부터 데이터를 입력받을때 사용하는 연산자로
    변수앞에 &를 붙여주면 된다. 실행시 입력창이 뜬다.

시나리오] 앞에서 작성한 프로그램을 치환연산자로 사원번호를 입력
받을 수 있도록 수정하시오. */
declare 
    type empTypes is record(
        emp_id employees.employee_id%type,
...
/* 예제6] 바인드변수 
    호스트환경에서 선언된 변수로써 비 PS/SQL 변수이다.
    호스트 환경에서 SQL> 프롬프트가 있는 상태로 명령어(쿼리문)를
    입력할 수 있는 상태를 말한다.
    형식]
        var 혹은 variable을 사용해서 생성
        var 변수명 자료형(크기) ; 
*/
...
begin
    --바인드변수는 일반변수와 구분하기 위해 :(콜론)을 추가한다.
    :return_var := 999;
