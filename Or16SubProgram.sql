...
서브프로그램(Sub Program)
    - PL/SQL에서는 프로시저와 함수라는 두가지 유형의 
        서브프로그램이 잇다.
    - select를 포함해서 모든 DML문을 이용하여 프로그래밍적인 요소를
        통해 사용 가능하다.
        
    - 프로시저 
        외부프로그램에서 호출하기 위해 정의한다.
        따라서 Java, JSP 등에서 간단한 호출로 복잡한 쿼리를
        실행할 수 있다.
        
    - 함수
        쿼리문의 일부분으로 사용하기 위해 정의한다.
        즉, 외부 프로그램에서 호출하는 경우는 거의 없다.
        
    - 트리거 
        프로시저의 일종으로 특정 테이블에 레코드의 변화가 있을경우
...
1. 저장 프로시저(Stored Procedure)
    ...
    형식]
        create [or replace] procedure 프로시저명
            ...
            /
※ 매개변수 성정시 자료형만 명시하고, 크기는 명시하지 않는다.


예제1] 사원의 급여를 가져와서 출력하는 프로시저 생성
시나리오] 100번 사원의 급여를 select하여 출력하는 저장프로시저를 
생성하시오. */
...
begin
    ...
    --결과를 내부에서 출력
    dbms_output.put_line('사원번호100의 급여는 : '|| v_salary ||
                            '입니다.');
end;
/
--실행시 데이터 사전에 저장만 되고 실행결과가 출력되진 않는다.

/* 데이터 사전에서 확인할 때는 '대문자'로 저장되므로 아래와 같이
upper함수를 사용해야한다. */
...
시나리오] 사원의 이름을 매개변수로 받아서 사원테이블에서 레코드를
조회한 후 해당사원의 급여를 출력하는 프로시저를 생성 후 실행하시오.
해당 문제는 in파라미터를 받은후 처리한다.
사원이름(first_name) : Bruce, Neena
*/
/* 프로시저 생성시 in파라미터를 설정. first_name 컬럼을 참조하는
형식으로 선언 */
...
name like upper('%pcd_in_param_salary%');
...
두가지 형식의 파라미터를 정의. 일반변수, 참조변수를 사용하여 선언함.
...
begin
    ...
    from employees where first_name = param_name;
    /* 프로시저에서는 별도로 return을 명시하지 않는다. 실행이
    종료되면 out파라미터에 할당된 값이 자동으로 반환된다. */
end;
/

--호스트 환경에서 바인드 변수를 선언한다.(variable도 사용가능)
var v_salary varchar2(30);
/* 프로시저 호출시 각각의 파라미터를 사용한다. 특히 바인드변수는 
:을 붙여서 일반 변수와 구분한다. Out파라미터인 param_salary에
저장된 값이 v_salary로 전달된다. */
execute pcd_out_param_salary('Nancy', :v_salary);
...
/*
시나리오] 사원번호와 급여를 매개변수로 전달받아 해당사원의 
급여를 수정하고, 실제 수정된 행의 갯수를 반환받아서 출력하는 
프로시저를 작성하시오.
...
begin
    ...
    /*
    SQL%notFound
        쿼리실행 후 적용된 행이 없을 경우 true를 반환한다.
        Found는 반대의 경우를 반환함.
    SQL%RowCount
    ...
    */
    ...
    /* 행의 변화가 있는 insert, update, delete 쿼리를 실행한 경우
    반드시 commit을 해야 실제 테이블에 적용되어 Oracle외부에서
    확인할 수 있다. */
    commit;
end;
/
...
것이좋다. 따라서 returnVal을 varchar2(20)으로 설정한다. */
...
select * from trigger_dept_backup2; --레코드 없음
...
begin
    ...
    if Inserting then
        dbms_output.put_line('insert 트리거 발동됨');
        /*
...
/*
생성2 : 오리지날테이블에 업데이트 이후 테이블(문장) 단위로 발생되는 트리거생성*/
create or replace trigger trigger_update_test
    after update
    on trigger_dept_original
    /*** for each row ***/ --주석 처리
begin
    if updating then
        ...
        insert into trigger_dept_backup
        values (
            /* 테이블 단위 트리거에서는 임시 테이블을 사용할 수 없다.
            따라서, 임의의 값을 사용해야한다. 임시 테이블 사용시 
            에러가 발생된다. */
            /*** :old.department_id,
            :old.department_name,
            :old.manager_id,
