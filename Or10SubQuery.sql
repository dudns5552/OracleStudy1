...
*/
--1. 담당업무별 가장 높은 급여를 확인
select
    job_id, max(salary)
from employees group by job_id;

--group by로 그룹화 되어있으므로 단일컬럼은 사용할 수 없어 에러발생
select
    job_id, max(salary), first_name
from employees group by job_id;


--앞에서 나온 결과를 바탕으로 단순한 or조건문으로 쿼리문 작성
select 
...
/*
    결과로 인출된 2개의 급여 중 하나만 만족하면 되므로 복수행 연산자
    any를 이용해서 서브쿼리문을 작성하면된다.
    즉, 여기서는 급여를 6000이상을 받는 사람들이 인출된다.
*/
...
/*
    600보다도 크고, 동시에 13000보다도 커야하므로 결과적으로
    13000보다 큰 레코드만 인출한다. 결과 : 5명
*/
