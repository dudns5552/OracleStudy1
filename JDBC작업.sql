...
CREATE TABLE member (
    /* 아이디, 비번, 이름은 문자타입으로 정의 nott null로
    빈값을 허용하지 않는 컬럼으로 정의 */
...
만약, 자식테이블에 먼저 입력하면 '부모키가 없다'는 에러가 발생한다. */
...
시나리오] 매개변수로 회원아이디(문자열)을 받으면 첫문자를 제외한
        나머지 부분을 *로 변환하는 함수를 생성하지오
        실행예) oracle21c -> o********

substr(문자열 혹은 컬럼, 시작인덱스, 길이) 
    시작인덱스부터 길이만큼 잘라낸다 */
select substr('hongildong',1,1) from dual;
/* rpad(문자열 혹은 컬럼, 길이, 채울문자)
    문자열의 남은 길이를 문자로 채움 */
...
is retStr varchar2(50); /* 마스킹 후 반환된 문자열을 저장할 변수 선언*/
begin
...
select fillAsterik('dd') from dual;
...
begin
    ...
    --프로시저는 별도의 return없이 아웃파리미터에 값을 할당하기만 하면된다.
end;
/
...
--둘다 일치하는 경우 (인증성공)
execute MyMemberAuth('test', '0412', :member_auth);
print member_auth;
--아이디만 일치하는 경우
execute MyMemberAuth('test', '0412암호틀림', :member_auth);
print member_auth;
--회원정보가 없는 경우
execute MyMemberAuth('test아이디틀림', '0412', :member_auth);
print member_auth;
